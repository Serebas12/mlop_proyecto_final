...
with DAG(
    ...
    def ensure_history_table_fn():
        """Crea la tabla de historial de modelos si no existe."""
    ...
    def extract_data_fn():
        """Extrae y divide los datos de entrenamiento."""
        engine = create_engine(CLEAN_DB_URI)
    ...
    def evaluate_and_promote_fn(ti):
        """Evalúa el modelo y lo promueve si es mejor."""
        client = MlflowClient()
    ...
    def compute_shap_fn(ti):
        """Calcula valores SHAP para interpretabilidad."""
        ...
        shap.summary_plot(shap_values, X_train_trans[:MAX_SHAP], show=False)
        plt.tight_layout()
        plt.savefig(f"{SHARED_TMP}/shap_summary.png", bbox_inches="tight")
        plt.close()

        # Registrar en MLflow
        mlflow.log_artifact(f"{SHARED_TMP}/shap_summary.png", artifact_path="shap")
        ti.xcom_push("shap_uri", f"runs:/{run_id}/shap/shap_summary.png")
    ...
    def record_history_fn(ti):
        """Registra el experimento en la tabla de historial."""
        cmp = ti.xcom_pull(task_ids="evaluate_and_promote", key="cmp")
        if not cmp:
            raise AirflowSkipException("No se encontró información de métricas (cmp)")
